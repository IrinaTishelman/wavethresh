accessC <-
  function(wd.obj, level = wd.obj$nlevels, boundary = FALSE)
{
    ctmp <- class(wd.obj)
	...
	wd.obj$C[(offset.level + 1):(offset.level + n)]
    }
    else {
	n <- 2^(level - 1)
	wd.obj$C[(offset.level + 1 - first.level):
...
accessD <- function(wd.obj, level, boundary = FALSE)
{
    ...
    if(boundary) {
	n <- last.level - first.level + 1
	wd.obj$D[(offset.level + 1):(offset.level + n)]
    }
    else {
	n <- 2^(level - 1)
	wd.obj$D[(offset.level + 1 - first.level):
		       (offset.level + n - first.level)]
    }
}
...
##
	...
	nm.count<-length(names(unsquished))
		if(verbose)
			cat("CD\t")
		unsquished$xxxx <- uncompress.default(imwdc[[nm]], ver =
			verbose)
